...
import { View, Text, TouchableOpacity, Image, Modal, StatusBar, ScrollView, KeyboardAvoidingView, Platform, Alert } from 'react-native';
...
export default function MenuLayout({ children, activeRoute, pageTitle, hasUnsavedChanges = false }: MenuLayoutProps) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView className="flex-1 px-4 py-2 mt-2">
          <View className="flex-row justify-between mb-6 px-2">
            ...
            <Text className="text-[#cc0000] text-xl font-bold">
              {usuario || 'Usuário'}
            </Text>
          </View>
      ...
      >
        <View className="flex-1 flex-row">
          <View className="w-3/4 bg-[#dcdcdc] h-full p-6 pt-12 shadow-2xl">
            ...
            <TouchableOpacity className={`p-4 rounded-lg mb-4 ${getMenuClass('/orcamento')}`} onPress={() => handleNavegacao('/orcamento')}>
              <Text className={`text-lg font-bold text-center ${getTextClass('/orcamento')}`}>Novo Orçamento</Text>
            </TouchableOpacity>

            <TouchableOpacity className={`p-4 rounded-lg mb-4 ${getMenuClass('/orcamentos-fechados')}`} onPress={() => handleNavegacao('/orcamentos-fechados')}>
              <Text className={`text-lg font-bold text-center ${getTextClass('/orcamentos-fechados')}`}>Orçamentos Fechados</Text>
            </TouchableOpacity>

            <TouchableOpacity className={`p-4 rounded-lg mb-4 ${getMenuClass('/tabela-vencimento')}`} onPress={() => handleNavegacao('/tabela-vencimento')}>
              <Text className={`text-lg font-bold text-center ${getTextClass('/tabela-vencimento')}`}>Tabela de Vencimento</Text>
            </TouchableOpacity>

            <TouchableOpacity className="mt-auto flex-row justify-center items-center p-4" onPress={() => { setMenuAberto(false); router.replace('/'); }}>
              <Feather name="log-out" size={24} color="black" />
              <Text className="text-black text-lg font-bold ml-2">Sair</Text>
            </TouchableOpacity>
          </View>
          <TouchableOpacity className="w-1/4 bg-black/50" onPress={() => setMenuAberto(false)} activeOpacity={1}/>
        </View>
      </Modal>
    </SafeAreaView>
  );
